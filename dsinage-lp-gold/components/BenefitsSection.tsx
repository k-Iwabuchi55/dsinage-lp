...
// このコンポーネントはHeroから切り出された導入メリット部分です
// AboutSectionとの順序を簡単に入れ替えられるよう独立させています
export default function BenefitsSection() {
  ...
  const benefits = [
    ...
    },
    {
      number: '03',
      title: 'ブランド認知度の向上',
      description: '高品質な映像で企業イメージを向上。記憶に残るブランディング。'
    },
    {
      number: '04',
      title: '営業活動の効率化',
      description: '必要なタイミングに必要な内容を発信。狙いを定めた効果的なマーケティングを実現。'
    },
    {
      number: '05',
      title: '24時間稼働による集客力の最大化',
      description: '深夜・早朝も休まず情報発信。営業時間外の認知度向上で新規顧客を獲得。'
    }
  ]
    ...
    <section ref={ref} className="py-16 relative">
      <div className="section-padding">
        <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <p className="text-xl md:text-2xl text-brand-gold/80 mb-12 tracking-wider text-center">
            ～ 5つの導入メリット ～
          </p>
        </div>
        ...
        <div className={`max-w-6xl mx-auto transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          {/* モバイル: 1列、タブレット: 2列、デスクトップ: 3-2レイアウト */}
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {benefits.slice(0, 3).map((benefit) => (
                ...
                >
                  <div className="text-3xl md:text-4xl font-bold text-brand-gold mb-3" style={{ fontFamily: 'var(--font-playfair)' }}>
                    {benefit.number}
                  </div>
